...
- `client.ts` — `createGarageClient()` wraps `openapi-fetch` with the stored bearer token and base URL. Call `refreshClient()` after the user saves new credentials. Token + base URL are persisted in localStorage.
...
- `/settings` → `SettingsPage` — base URL and admin token config
